...
func (s *Service) handleHealthData(t *eventTopic, payload []byte, device *services.Device) {
	...
	s.writeToInfluxDB(device.Tenant.Name, point, fields)
...
func (s *Service) writeToInfluxDB(org string, point *write.Point, fields map[string]interface{}) {
	writeApi := s.influxClient.Client.WriteAPIBlocking(s.cfg.InfluxDB.Org, org)

	err := writeApi.WritePoint(context.Background(), point)
	if err != nil {
		log.Printf("Error write point to influxdb %s with data %s", err, fields)
	}
}
